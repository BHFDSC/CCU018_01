...
# Save and plot ----------------------------------------------------------------
trimester_Total <-ggplot(data = df[df$sens=="total",],
...
trimester_sens_covid <-ggplot(data = df[df$sens=="sens_covid",],
  ...
  scale_y_continuous(lim = c(0.25,5.5), breaks = c(0.5,1,2, 4), trans = "log")  +
  scale_x_continuous(lim = c(2,6), breaks = c(3, 4, 5), label = c("","", ""))  +
  ggtitle("Censored \nat COVID-19 diagnosis") +
  scale_fill_manual(values = levels(df$colour)[1:3],) +
  scale_color_manual(values = levels(df$colour)[1:3],) +
  labs(x ="", y = "HR and 95% CI") +
...
grid.arrange(trimester_Total, trimester_sens_covid, ncol=2)

trimester <- arrangeGrob(trimester_Total, trimester_sens_covid,  ncol=2)
