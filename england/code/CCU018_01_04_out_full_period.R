...
writeLines(c("event model adj exp estimate conf.low conf.high stad.error n.event subgroup"), paste0('D:/PhotonUser/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_full_period.csv'))
...
cox_model_2week <- function(k, y, z) {
  ...
  map2(cov_model, adjustment, function(x,i) {
    ...
    s <- summary(model)
    cat(paste0(substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(s$coefficients[1:2,], 1,
                                                                                              function(x) {
               ...
               collapse = '\n'), '\n', sep = '', file = paste0("D:/PhotonUser/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_full_period.csv"), append = TRUE)
    invisible(model)
  })
...
cox_model_full <- function(k, y, z) {
  ...
  lapply(split(survival_data, survival_data$dep_label), function(y)
    map2(cov_model, adjustment, function(x,i) {
      ...
      s <- summary(model)
      k <- s$coefficients[1,,drop=F]
      cat(paste0(substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(k, 1,                                                                             function(x) {
        paste0(" ", round(exp(x[1]), 3),
               ' ', round(exp(x[1] - 1.96 * x[3]), 3),
               ' ', round(exp(x[1] + 1.96 * x[3]), 3),
               " ", round((x[3]), 4),
...
library(readr)
library(plyr)
out <- read_table2("D:/PhotonUser/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_full_period.csv")
nrow(out)
out <- out[which(out$n.event > 10),]
out$n.event <- round_any(out$n.event, 5)
nrow(out)
data.table::fwrite(out1,"D:/PhotonUser/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_full_period.csv")
