...
## against COVID-19 (1st dose) and adverse pregnancy outcomes
##
## By : Elena Raffetti
##
################################################################################

writeLines(c("sens event model adj exp estimate conf.low conf.high stad.error n.event subgroup"), paste0('C:/ProgramData/UserDataFolders/S-1-5-21-379329792-2760669353-3648467936-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/1st_vaccination_out_HRs_full_period.csv'))
...
if (q == "sens_no_early_preg"){
  ...
  data <- data[which(data$VACC_1ST_DUR_DATE >'2021-06-18' | data$VACC_1ST_DUR_FLAG==0),]
  nrow(data)
}

# define vector with sensitivity analysis information to include in the output
# call in map2 functions
sens <- c(paste0(q))
...
cox_model_2week <- function(k, y, z) {
  ...
  # prepare dataframe for those  without vaccinaion ----------------------------
  ...
  map2(cov_model, adjustment, function(x,i) {
    ...
    cat(paste0(sens,' ',substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(s$coefficients[1:2,], 1,
               ...
               collapse = '\n'), '\n', sep = '', file = paste0("C:/ProgramData/UserDataFolders/S-1-5-21-379329792-2760669353-3648467936-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/1st_vaccination_out_HRs_full_period.csv"), append = TRUE)
    invisible(model)
  })

if (sens == "total" | sens == "sens_covid"){
  ...
  lapply(split(survival_data, survival_data$parity_label), function(y)
    map2(cov_model, adjustment, function(x,i) {
      ...
      model$call$formula <- f
      s <- summary(model)
      cat(paste0(sens,' ',substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(s$coefficients[1:2,], 1,                                                                             function(x) {
        paste0(" ", round(exp(x[1]), 3),
               ' ', round(exp(x[1] - 1.96 * x[3]), 3),
               ' ', round(exp(x[1] + 1.96 * x[3]), 3),
               " ", round((x[3]), 4),
...
cov_model <-  c("vacc_covid",
                "vacc_covid + COV_AGE  +  age_sq  + COV_DEPRIVATION ",
                "vacc_covid + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_no_hx_preg) + bs(calendar_month) + region")
...
lapply(split(survival_data, survival_data$parity_label), function(y)
  ...
  cov_model <-  c("vacc_covid",
                  "vacc_covid + COV_AGE  +  age_sq  + COV_DEPRIVATION ",
                  "vacc_covid + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_no_ethn) + bs(calendar_month) + region")
  ...
  cov_model <-  c("vacc_covid",
                  ...
                  "vacc_covid + COV_AGE  +  age_sq  + bs(pr_score_full) + bs(calendar_month) + region")
  ...
  # set parameters--------------------------------------------------------------
  cov_model <-  c("vacc_covid",
                  "vacc_covid + COV_DEPRIVATION ",
                  "vacc_covid + COV_DEPRIVATION + bs(pr_score_full) + bs(calendar_month) + region")
  ...
  cov_model <-  c("vacc_covid",
                  "vacc_covid + COV_AGE  +  age_sq  + COV_DEPRIVATION",
                  "vacc_covid + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_no_health_cond) + bs(calendar_month) + region")
...
out <- read_table2("C:/ProgramData/UserDataFolders/S-1-5-21-379329792-2760669353-3648467936-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/1st_vaccination_out_HRs_full_period.csv")
nrow(out)
...
data.table::fwrite(out2,"C:/ProgramData/UserDataFolders/S-1-5-21-379329792-2760669353-3648467936-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/1st_vaccination_out_HRs_full_period.csv")
