...
## against COVID-19 (1st dose) and adverse pregnancy outcomes stratified for
...
writeLines(c("sens tri event model adj exp estimate conf.low conf.high stad.error n.event subgroup"), paste0('C:/ProgramData/UserDataFolders/S-1-5-21-3444344212-1812383397-2900750023-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/1st_vaccination_out_HRs_trimester.csv'))
...
sensitivity_analyses <- function(p) {
...
if (p == "sens_covid"){
  ...
  data <- data %>%
    mutate     (VACC_1ST_DUR_FLAG  = case_when(data$VACC_1ST_DUR_FLAG ==0  ~ 0 ,
                                               data$VACC_1ST_DUR_FLAG ==1 & data$EXP_COVID_BIN ==0 ~ 1 ,
                                               data$VACC_1ST_DUR_FLAG ==1 & data$EXP_COVID_BIN ==1 & (data$EXP_DUR_COVID_1ST_DATE < data$VACC_1ST_DUR_DATE) ~ 0 ,
                                               data$VACC_1ST_DUR_FLAG ==1 & data$EXP_COVID_BIN ==1 & (data$EXP_DUR_COVID_1ST_DATE >= data$VACC_1ST_DUR_DATE) ~ 1 ,
    )
    )
  ...
  data$EXP_VACC_TRIMESTER[data$VACC_1ST_DUR_FLAG==0] <- 0
  
  data_check <- data%>% select (PREG_START_DATE, COHORT_END_DATE, EXP_COVID_BIN, EXP_DUR_COVID_1ST_DATE,
                                VACC_1ST_DUR_FLAG, VACC_1ST_DUR_DATE
  )
}
...
# start trimester analysis function --------------------------------------------
trimester <- function(q) {
  
  if (q == "trimester1"){
    
    data <- data %>%
      mutate     (VACC_1ST_DUR_FLAG = case_when(data$EXP_VACC_TRIMESTER ==0 |  data$EXP_VACC_TRIMESTER ==2 |  data$EXP_VACC_TRIMESTER ==3 ~ 0,
                                            data$EXP_VACC_TRIMESTER ==1 ~ 1,
                                             
      )
      )
    
    data <- data %>%
      mutate(COHORT_END_DATE = case_when(data$EXP_VACC_TRIMESTER ==0 |  data$EXP_VACC_TRIMESTER ==1 ~  data$COHORT_END_DATE,
                                       data$EXP_VACC_TRIMESTER ==2 |  data$EXP_VACC_TRIMESTER ==3 ~  data$VACC_1ST_DUR_DATE
      )
      )
  }
  ...
  if (q == "trimester2"){
    ...
    data <- data %>%
      mutate     (VACC_1ST_DUR_FLAG = case_when(data$EXP_VACC_TRIMESTER ==0 |  data$EXP_VACC_TRIMESTER ==1 |  data$EXP_VACC_TRIMESTER ==3 ~ 0,
                                            data$EXP_VACC_TRIMESTER ==2 ~ 1,
                                            
      )
      )
    
    data <- data %>%
      mutate(COHORT_END_DATE = case_when(data$EXP_VACC_TRIMESTER ==0 |  data$EXP_VACC_TRIMESTER ==2 ~  data$COHORT_END_DATE,
                                       data$EXP_VACC_TRIMESTER ==1 |  data$EXP_VACC_TRIMESTER ==3 ~  data$VACC_1ST_DUR_DATE
      )
      )
  }
  ...
  if (q == "trimester3"){
    ...
    data <- data %>%
      mutate     (VACC_1ST_DUR_FLAG = case_when(data$EXP_VACC_TRIMESTER ==0 |  data$EXP_VACC_TRIMESTER ==1 |  data$EXP_VACC_TRIMESTER ==2 ~ 0,
                                            data$EXP_VACC_TRIMESTER ==3 ~ 1,
                                            
      )
      )
    
    data <- data %>%
      mutate(COHORT_END_DATE = case_when(data$EXP_VACC_TRIMESTER ==0 |  data$EXP_VACC_TRIMESTER ==3 ~  data$COHORT_END_DATE,
                                       data$EXP_VACC_TRIMESTER ==1 |  data$EXP_VACC_TRIMESTER ==2 ~  data$VACC_1ST_DUR_DATE
      )
      )
  }
  
trimester <- c(paste0(q))
...
cox_model_2week <- function(k, y, z) {
  ...
  vaccinated_cases$delta_vacc_covid<-as.numeric(difftime(vaccinated_cases$vacc_covid_date, vaccinated_cases$PREG_START_DATE, unit="days"))
  ...
  data_surv$days_to_expo<-as.numeric(difftime(data_surv$vacc_covid_date, data_surv$PREG_START_DATE, unit="days"))
  ...
  map2(cov_model, adjustment, function(x,i) {
    ...
    s <- summary(model)
    cat(paste0(sens,' ',trimester,' ',substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(s$coefficients[1:2,], 1,
                                                                               function(x) {
               ...
               collapse = '\n'), '\n', sep = '', file = paste0("C:/ProgramData/UserDataFolders/S-1-5-21-3444344212-1812383397-2900750023-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/1st_vaccination_out_HRs_trimester.csv"), append = TRUE)
    invisible(model)
  })
...
cox_model_full <- function(k, y, z) {
  ...
  # combine unvaccianted and vaccinated exposure dataframes ----------------------
  ...
  cov_model <-  c("vacc_covid",
                  "vacc_covid + COV_AGE  +  age_sq  + COV_DEPRIVATION ",
                  "vacc_covid + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_full) + bs(calendar_month) + region")
  ...
  map2(cov_model, adjustment, function(x,i) {
    ...
    cat(paste0(sens,' ',trimester,' ',substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(k, 1,
                                                                              function(x) {
                                                                                paste0(" ", round(exp(x[1]), 3),
                                                                                       ' ', round(exp(x[1] - 1.96 * x[3]), 3),
                                                                                       ' ', round(exp(x[1] + 1.96 * x[3]), 3),
...
out <- read_table2("C:/ProgramData/UserDataFolders/S-1-5-21-3444344212-1812383397-2900750023-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/1st_vaccination_out_HRs_trimester.csv")
nrow(out)
out <- out[which(out$n.event > 10),]
out$n.event <- round_any(out$n.event, 5)
nrow(out)
data.table::fwrite(out,"C:/ProgramData/UserDataFolders/S-1-5-21-3444344212-1812383397-2900750023-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/1st_vaccination_out_HRs_trimester.csv")
