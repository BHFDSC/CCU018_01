...
## and adverse pregnancy outcomes stratified for trimester
##
## By : Elena Raffetti
##
################################################################################

writeLines(c("sens tri event model adj exp estimate conf.low conf.high stad.error n.event subgroup"), paste0('C:/ProgramData/UserDataFolders/S-1-5-21-3444344212-1812383397-2900750023-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_trimester.csv'))
...
# analysis stratified for trimester -------------------------------------------- 
# right censor if the exposure occurs in another trimester
trimester <- function(q) {
  
  if (q == "trimester1"){

    data <- data %>%
      mutate(EXP_COVID_BIN = case_when(data$EXP_TRIMESTER==0 |  data$EXP_TRIMESTER==2 |  data$EXP_TRIMESTER==3 ~ 0,
                                            data$EXP_TRIMESTER==1 ~ 1,
                                            
      )
      )
    
    data <- data %>%
      mutate(COHORT_END_DATE = case_when(data$EXP_TRIMESTER==0 |  data$EXP_TRIMESTER==1 ~  data$COHORT_END_DATE,
                                       data$EXP_TRIMESTER==2 |  data$EXP_TRIMESTER==3 ~  data$EXP_DUR_COVID_1ST_DATE
      )
      )
  }
  ...
  if (q == "trimester2"){
    ...
    data <- data %>%
      mutate     (EXP_COVID_BIN = case_when(data$EXP_TRIMESTER==0 |  data$EXP_TRIMESTER==1 |  data$EXP_TRIMESTER==3 ~ 0,
                                            data$EXP_TRIMESTER==2 ~ 1,
                                            
      )
      )
    
    data <- data %>%
      mutate(COHORT_END_DATE = case_when(data$EXP_TRIMESTER==0 |  data$EXP_TRIMESTER==2 ~  data$COHORT_END_DATE,
                                       data$EXP_TRIMESTER==1 |  data$EXP_TRIMESTER==3 ~  data$EXP_DUR_COVID_1ST_DATE
      )
      )
  }
  ...
  if (q == "trimester3"){
    ...
    data <- data %>%
      mutate     (EXP_COVID_BIN = case_when(data$EXP_TRIMESTER==0 |  data$EXP_TRIMESTER==1 |  data$EXP_TRIMESTER==2 ~ 0,
                                            data$EXP_TRIMESTER==3 ~ 1,
                                            
      )
      )
    
    data <- data %>%
      mutate(COHORT_END_DATE = case_when(data$EXP_TRIMESTER==0 |  data$EXP_TRIMESTER==3 ~  data$COHORT_END_DATE,
                                       data$EXP_TRIMESTER==1 |  data$EXP_TRIMESTER==2 ~  data$EXP_DUR_COVID_1ST_DATE
      )
      )
  }
...
vars <- names(data_ps) %in% c("ID", "exposure", "COV_HX_OTHER_DIS", "COV_HX_CVD_HEM", "COV_HX_DEPRESSION_FLAG", "COV_ETHNICITY_3lev", 
                              ...
                              "COV_HX_PREGNANCY_MAX", "COV_SMOKING_STATUS", "OUT_BIN_BIRTH_MULTI_GEST_MAX","PREG_START_DATE_ESTIMATED")
...
data_ps_birth <- data_ps_birth [which(data_ps_birth$OUT_BIN_BIRTH_MULTI_GEST_MAX==0),]

prop_model_full <- glm(exposure ~  COV_HX_PREGNANCY_MAX  + OUT_BIN_BIRTH_MULTI_GEST_MAX + COV_HX_HYPERTENSIVE_DIS + COV_HX_DIABETES_DIS + COV_HX_OTHER_DIS  + COV_HX_BMI_OBESITY_FLAG + COV_HX_CVD_HEM  + COV_HX_DEPRESSION_FLAG
...
cox_model_2week <- function(k, y, z) {
  ...
  map2(cov_model, adjustment, function(x,i) {
    ...
    cat(paste0(sens,' ',trimester,' ',substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(s$coefficients[1:2,], 1,
               ...
               collapse = '\n'), '\n', sep = '', file = paste0("C:/ProgramData/UserDataFolders/S-1-5-21-3444344212-1812383397-2900750023-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_trimester.csv"), append = TRUE)
    invisible(model)
  })
...
library(readr)
library(plyr)
out <- read_table2("C:/ProgramData/UserDataFolders/S-1-5-21-3444344212-1812383397-2900750023-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_trimester.csv")
View(out)
nrow(out)
out <- out[which(out$n.event > 10),]
out$n.event <- round_any(out$n.event, 5)
nrow(out)
data.table::fwrite(out,"C:/ProgramData/UserDataFolders/S-1-5-21-3444344212-1812383397-2900750023-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_trimester.csv")
