...
# cohort
ccu018_cohort_tbl <- tbl(con, in_schema("", "ccu018_01_out_cohort"))
cohort <- ccu018_cohort_tbl %>%
  collect()

# covariates
ccu018_covariates_tbl <- tbl(con, in_schema("", "ccu018_01_out_covariates"))
covariates <- ccu018_covariates_tbl %>%
  collect()

# covid exposure
ccu018_covid_tbl <- tbl(con, in_schema("", "ccu018_01_out_exposures"))
exposure <- ccu018_covid_tbl %>%
  collect()

# outcomes during pregnancy
ccu018_outcomes_dur_preg_tbl <- tbl(con, in_schema("", "ccu018_01_out_outcomes_dur_preg"))
outcomes_during_preg <- ccu018_outcomes_dur_preg_tbl %>%
  collect()

# outcomes post pregnancy
ccu018_outcomes_post_preg_tbl <- tbl(con, in_schema("", "ccu018_01_out_outcomes_post_preg"))
outcomes_post_preg <- ccu018_outcomes_post_preg_tbl %>%
  collect()

# outcomes at birth
ccu018_outcomes_at_birth_tbl <- tbl(con, in_schema("", "ccu018_01_out_outcomes_at_birth"))
outcomes_birth <- ccu018_outcomes_at_birth_tbl %>%
  collect()

# skinny table
ccu018_skinny_tbl <- tbl(con, in_schema("", "ccu018_01_out_skinny"))
