...
# MAGIC %md # CCU018_01-D03-cohort
# MAGIC  
# MAGIC **Description** This notebook creates the cohort table for the CCU018_01 project, which includes: preparation of the HES APC, HES APC Maternity, GDPPR, and Deaths tables; filtering to delivery records; identifying appropriate delivery records; adding relevant dates and applying project inlcusion criteria (i.e., delivery date filter, pregnancy start date filter, gdppr filter, first delivery filter). A flow diagram (table) was used to record each stage. 
# MAGIC  
# MAGIC **Author(s)** Tom Bolton, John Nolan, Elena Raffetti (based on work from CCU002)
# MAGIC
# MAGIC **Data input** hes_apc, hes_apc_mat, gdppr, deaths
# MAGIC
# MAGIC **Data output** ccu018_01_tmp_cohort \
# MAGIC (ccu018_01_tmp_hes_apc_mat_del, ccu018_01_tmp_cohort_del, ccu018_01_tmp_cohort_flow, ccu018_01_tmp_cohort_multiple)

# COMMAND ----------

# todo: KH mentioned Mother-baby linkage could be used to reduce the amount of missing information in the mother's record for GESTAT.
...
# todo: investigate small number of individuals with missing DOD, but with a death record
...
# ER confirmed the exclusion of unfinished maternity episodes
...
# todo: restructure cohort and cohort_deliveries_clean

# COMMAND ----------

# MAGIC %run "./CCU018_01-D03a-cohort_deliveries_clean"
...
# check consistency with previous delivery date for small number of individuals with multiple delivery events, noting
...
# check
tmpc = count_var(tmp1x, 'PERSON_ID_DEID', ret=1, df_desc='original', indx=7); print()
...
tmpc1 = spark.table(f'{dbc}.{proj}_tmp_flow_cohort')
display(tmpc1)
...
# data for JN 202204 webinar plots (also reproduced below)
tmp3a = tmp3\
