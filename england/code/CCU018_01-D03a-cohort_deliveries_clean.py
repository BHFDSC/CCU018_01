...
# note: this markdown cell has been intentionally commented out to avoid this cell being displayed in the other notebooks running this notebook.

# %md # CCU018_01-D03a-cohort_deliveries_clean
 
# **Description** This notebook cleans the delivery records - identifying appropriate delivery records for the CCU018_01 project, which includes dropping duplicate records, dropping records without evidence of (or useable information) for a delivery, dropping records failing quality assurance, combining records within 28 days, and dropping records with an interval of <210 days. A flow diagram (table) was used to record each stage. 
 
# **Author(s)** Tom Bolton, John Nolan, Elena Raffetti (based on work from CCU002)

# **Data input** ccu018_01_tmp_hes_apc_mat_del

# **Data output** ccu018_01_tmp_hes_apc_mat_del_clean\
# (ccu018_01_tmp_flow_hes_apc_mat_del_clean)
...
# note: for simplicity, only _1 in the baby tail has been used. Exploratory analysis revealed that _2 to _9 did not provide additional evidence of delivery 
# todo: investigate higher tail indices for conflicts or additional info (<1% of records).
...
# note: excluding NUMTAILB - default is 1, not providing evidence of delivery
# todo: consider excluding dummy and implausible dates (e.g., ANASDATE = 1801-01-01 (<1.2%)) 
...
# note: separate data step for the below as need columns of tmp2
tmp2 = tmp2\
...
# DBTITLE 1,Relationship between GESTAT and BIRWEIT for providers RAE and RJ6 appears suspect
tmpp = tmp4\
...
# drop provider codes with severe departures from the expected GESTAT vs BIRWEIT relationship
tmp5 = tmp4\
...
# note: added EPIKEY to orderBy to ensure results can be reproduced in the event of ties
...
# note: upon inspection, combining chains of 28-day separated records is more appropriate (impacts <30 records [i.e., _flag_gt28_2])
...
# note: for MATAGE, <None> did not work with isin, so, as a quick fix, used <99> as an artificial dummy value to avoid a code rewrite
_dict = {
  'BIRWEIT_1': 9999,
  'GESTAT_1': 99,
  'MATAGE': 99,
