...
## and adverse pregnancy outcomes - sensitivity analyses
##
## By : Elena Raffetti
##
################################################################################

writeLines(c("sens event model adj exp estimate conf.low conf.high stad.error n.event subgroup"), paste0('C:/ProgramData/UserDataFolders/S-1-5-21-379329792-2760669353-3648467936-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_sens.csv'))
...
if (q == "sens_hosp_no"){
  ...
  data <- data %>%
    mutate     (COHORT_END_DATE = case_when(  is.na(data$EXP_DUR_HOSP) |  data$EXP_DUR_HOSP==0 ~  data$COHORT_END_DATE,
                                              data$EXP_DUR_HOSP==1 ~   data$EXP_DUR_COVID_1ST_DATE
    )
    )
}
...
if (q == "sens_hosp_yes"){
  ...
  data$EXP_DUR_COVID_1ST_DATE <- as.Date(data$EXP_DUR_COVID_1ST_DATE, "%d/%m/%Y")
  data <- data %>%
    mutate     (COHORT_END_DATE = case_when(  is.na(data$EXP_DUR_HOSP) |  data$EXP_DUR_HOSP==1 ~  data$COHORT_END_DATE,
                                              data$EXP_DUR_HOSP==0 ~   data$EXP_DUR_COVID_1ST_DATE
    )
    )
}

if (q == "sens_pandemic_initiation"){
  data$PREG_START_DATE <- as.Date(data$PREG_START_DATE, "%d/%m/%Y")
  data <- data[which(data$PREG_START_DATE >= '2020-03-11'),]
  data$COHORT_END_DATE <- as.Date(data$COHORT_END_DATE, "%d/%m/%Y")
}

if (q == "sens_pandemic_initiation_first_year"){
  data$PREG_START_DATE <- as.Date(data$PREG_START_DATE, "%d/%m/%Y")
  data$COHORT_END_DATE <- as.Date(data$COHORT_END_DATE, "%d/%m/%Y")
  data <- data[which(data$PREG_START_DATE >= '2020-03-11'),]
  data <- data[which(data$COHORT_END_DATE <= '2021-03-11'),]
  
  # excluded vaccinated women
  data <- data[which(data$VACC_1ST_DATE >= '2021-03-11'),]
}

if (q == "sens_pandemic_initiation_before_pandemic"){
  data$PREG_START_DATE <- as.Date(data$PREG_START_DATE, "%d/%m/%Y")
  data <- data[which(data$PREG_START_DATE < '2020-03-11'),]
...
if (q == "post_vaccination_anytime_vaccinated"){
  data$PREG_START_DATE <- as.Date(data$PREG_START_DATE, "%d/%m/%Y")
  data <- data[which(data$PREG_START_DATE >= '2020-12-08'),]
...
if (q == "post_vaccination_anytime_vaccinated_no_early_vacc"){
  data$PREG_START_DATE <- as.Date(data$PREG_START_DATE, "%d/%m/%Y")
  data <- data[which(data$PREG_START_DATE >= '2020-12-08'),]
...
if (q == "post_vaccination_anytime_non_vaccinated"){
  data$PREG_START_DATE <- as.Date(data$PREG_START_DATE, "%d/%m/%Y")
  data <- data[which(data$PREG_START_DATE >= '2020-12-08'),]
  
  data$COHORT_END_DATE <- as.Date(data$COHORT_END_DATE, "%d/%m/%Y")
  
  table(data$VACC_1ST_FLAG)
  data <- data[which(data$VACC_1ST_FLAG == 0),]
}

if (q == "ind_lab"){
  # 1)
  table(data$PREG_START_DATE_ESTIMATED)
  data <- data[which(data$OUT_BIRTH_IND_LAB==0),]
}
...
cox_model_2week <- function(k, y, z) {
  ...
  # setup for 0 follow-up time sensitivity analysis  ---------------------------
  if (q == "no_event_day0"){
    data$delta_event<-as.numeric(difftime(data$covid_inf_date, data$overall_pregn_event_date, unit="days"))
    data <- data[which(data$delta_event!=0 | is.na(data$delta_event)),]
  }
  ...
  map2(cov_model, adjustment, function(x,i) {
    ...
    cat(paste0(sens,' ',substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(s$coefficients[1:2,], 1,
               ...
               collapse = '\n'), '\n', sep = '', file = paste0("C:/ProgramData/UserDataFolders/S-1-5-21-379329792-2760669353-3648467936-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_sens.csv"), append = TRUE)
    invisible(model)
  })
  
  # stratified analyses---------------------------------------------------------
  if (sens== "pre_vaccination" | sens == "post_vaccination"){

    cov_model <-  c("E1 + E2",
                    "E1 + E2 + COV_AGE  +  age_sq  + COV_DEPRIVATION ",
                    "E1 + E2 + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_no_hx_preg) + bs(calendar_month) + region")
    ...
    cov_model <-  c("E1 + E2",
                    "E1 + E2 + COV_AGE  +  age_sq  + COV_DEPRIVATION ",
                    "E1 + E2 + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_no_ethn) + bs(calendar_month) + region")
    ...
    cov_model <-  c("E1 + E2",
                    "E1 + E2 + COV_AGE  +  age_sq",
                    "E1 + E2 + COV_AGE  +  age_sq  + bs(pr_score_full) + bs(calendar_month) + region")
    ...
    cov_model <-  c("E1 + E2",
                    "E1 + E2 + COV_DEPRIVATION ",
                    "E1 + E2 + COV_DEPRIVATION + bs(pr_score_full) + bs(calendar_month) + region")
    ...
    cov_model <-  c("E1 + E2",
                    "E1 + E2 + COV_AGE  +  age_sq  + COV_DEPRIVATION ",
                    "E1 + E2 + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_no_health_cond) + bs(calendar_month) + region")
...
cox_model_full <- function(k, y, z) {
  ...
  cov_model <-  c("covid_inf",
                  "covid_inf + COV_AGE  +  age_sq  + COV_DEPRIVATION ",
                  "covid_inf + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_no_hx_preg) + bs(calendar_month) + region")
  ...
  lapply(split(survival_data, survival_data$parity_label), function(y)
    map2(cov_model, adjustment, function(x,i) {
      ...
      s <- summary(model)
      k <- s$coefficients[1,,drop=F]
      cat(paste0(sens,' ',substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(k, 1, function(x) {
        paste0(" ", round(exp(x[1]), 3),
               ' ', round(exp(x[1] - 1.96 * x[3]), 3),
               ' ', round(exp(x[1] + 1.96 * x[3]), 3),
               " ", round((x[3]), 4),
  ...
  cov_model <-  c("covid_inf",
                  "covid_inf + COV_AGE  +  age_sq  + COV_DEPRIVATION ",
                  "covid_inf + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_no_ethn) + bs(calendar_month) + region")
  ...
  cov_model <-  c("covid_inf",
                  "covid_inf + COV_AGE  +  age_sq  ",
                  "covid_inf + COV_AGE  +  age_sq  + bs(pr_score_full)  + bs(calendar_month) + region")
  ...
  cov_model <-  c("covid_inf",
                  "covid_inf + COV_DEPRIVATION ",
                  "covid_inf + COV_DEPRIVATION + bs(pr_score_full) + bs(calendar_month) + region")
  ...
  cov_model <-  c("covid_inf",
                  "covid_inf + COV_AGE  +  age_sq  + COV_DEPRIVATION ",
                  "covid_inf + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_no_health_cond) + bs(calendar_month) + region")
...
sensitivity_analyses("sens_hosp_no")
sensitivity_analyses("sens_hosp_yes")
sensitivity_analyses("sens_missing")
sensitivity_analyses("sens_pandemic_initiation")
sensitivity_analyses("sens_pandemic_initiation_first_year")
sensitivity_analyses("sens_pandemic_initiation_before_pandemic")
sensitivity_analyses("post_vaccination_anytime_non_vaccinated")
sensitivity_analyses("post_vaccination_anytime_vaccinated_no_early_vacc")
sensitivity_analyses("post_vaccination_anytime_vaccinated")
sensitivity_analyses("no_event_day0")
sensitivity_analyses("ind_lab")
sensitivity_analyses("ino_event_day0")

#include models with more than 10 events----------------------------------------
library(readr)
library(plyr)
out <- read_table2("C:/ProgramData/UserDataFolders/S-1-5-21-379329792-2760669353-3648467936-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_sens.csv")

out <- out[which(out$n.event > 10),]
nrow(out)
out$n.event <- round_any(out$n.event, 5)

data.table::fwrite(out,"C:/ProgramData/UserDataFolders/S-1-5-21-379329792-2760669353-3648467936-1012/My Files/Home Folder/collab/CCU018/CCU018_01/output/out_HRs_sens.csv")
