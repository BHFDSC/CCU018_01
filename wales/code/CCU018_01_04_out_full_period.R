...
writeLines(c("analysis event model adj exp estimate conf.low conf.high stad.error n.event subgroup"), paste0('S:/WMCC - WMCC - Wales Multi-morbidity Cardiovascular Covid-19 UK (0911)/CCU018/Scripts/R_scripts/output/out_dur_HRs_full_period.csv'))

# start function ---------------------------------------------------------------
infection_analysis <- function(q) {
...
cox_model_2week <- function(k, y, z) {
  ...
  map2(cov_model, adjustment, function(x,i) {
    ...
    s <- summary(model)
    cat(paste0(period_analysis,' ',substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(s$coefficients[1:2,], 1,
                                                                                              function(x) {
               ...
               collapse = '\n'), '\n', sep = '', file = paste0("S:/WMCC - WMCC - Wales Multi-morbidity Cardiovascular Covid-19 UK (0911)/CCU018/Scripts/R_scripts/output/out_dur_HRs_full_period.csv"), append = TRUE)
    invisible(model)
  })
...
cox_model_full <- function(k, y, z) {
  ...
  cov_model <-  c("covid_inf",
                  ...
                  "covid_inf + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_full) + bs(calendar_month)")
  ...
  map2(cov_model, adjustment, function(x,i) {
    ...
    cat(paste0(period_analysis,' ',substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(k, 1,
                                                                              function(x) {
                                                                                paste0(" ", round(exp(x[1]), 3),
                                                                                       ' ', round(exp(x[1] - 1.96 * x[3]), 3),
                                                                                       ' ', round(exp(x[1] + 1.96 * x[3]), 3),
...
infection_analysis("total")
infection_analysis("pre_vaccination")
infection_analysis("vaccination")

#include model with more than 10 events-----------------------------------------
library(readr)
library(plyr)
out <- read_table2("S:/WMCC - WMCC - Wales Multi-morbidity Cardiovascular Covid-19 UK (0911)/CCU018/Scripts/R_scripts/output/out_dur_HRs_full_period.csv")
View(out)
nrow(out)
out <- out[which(out$n.event > 10),]
out$n.event <- round_any(out$n.event, 5)
nrow(out)
data.table::fwrite(out,"S:/WMCC - WMCC - Wales Multi-morbidity Cardiovascular Covid-19 UK (0911)/CCU018/Scripts/R_scripts/output/out_dur_HRs_full_period_to_export.csv")
