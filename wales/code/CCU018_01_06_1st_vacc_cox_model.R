...
writeLines(c("sens event model adj exp estimate conf.low conf.high stad.error n.event subgroup"), paste0('S:/WMCC - WMCC - Wales Multi-morbidity Cardiovascular Covid-19 UK (0911)/CCU018/Scripts/R_scripts/output/out_vaccination.csv'))
...
cox_model_2week <- function(k, y, z) {
  ...
  #with_expo <- subset(with_expo, select = -c(id))
  ...
  cov_model <-  c("E1 + E2",
                  ...
                  "E1 + E2 + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_full) + bs(calendar_month)")
  ...
  map2(cov_model, adjustment, function(x,i) {
    ...
    cat(paste0(sens,' ',substring(event_model,1,10),' ',analysis,' ', i,' ',cov, apply(s$coefficients[1:2,], 1,
               ...
               collapse = '\n'), '\n', sep = '', file = paste0("S:/WMCC - WMCC - Wales Multi-morbidity Cardiovascular Covid-19 UK (0911)/CCU018/Scripts/R_scripts/output/out_vaccination.csv"), append = TRUE)
    invisible(model)
  })
...
cox_model_full <- function(k, y, z) {
  ...
  cov_model <-  c("vacc_covid",
                  ...
                  "vacc_covid + COV_AGE  +  age_sq  + COV_DEPRIVATION + bs(pr_score_full) + bs(calendar_month)")
...
library(readr)
library(plyr)
out <- read_table2("S:/WMCC - WMCC - Wales Multi-morbidity Cardiovascular Covid-19 UK (0911)/CCU018/Scripts/R_scripts/output/out_vaccination.csv")
nrow(out)
View(out)
out <- out[which(out$n.event > 10),]
out$n.event <- round_any(out$n.event, 5)
nrow(out)
data.table::fwrite(out,"S:/WMCC - WMCC - Wales Multi-morbidity Cardiovascular Covid-19 UK (0911)/CCU018/Scripts/R_scripts/output/out_vaccination_to_export.csv")
